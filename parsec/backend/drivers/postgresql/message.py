...
from pypika import PostgreSQLQuery as Query, Parameter, Order, functions as fn
...
class PGMessageComponent(BaseMessageComponent):
    ...
    _q_get_messages = (
        Query.from_(t_message)
        .select(
            q_device(_id=t_message.sender).select("device_id"), t_message.timestamp, t_message.body
        )
        .where(
            t_message.recipient
            == q_user_internal_id(organization_id=Parameter("$1"), user_id=Parameter("$2"))
    ...
    async def send(
        ...
        async with self.dbh.pool.acquire() as conn, conn.transaction():
            index = await conn.fetchval(
                self._q_insert_message, organization_id, recipient, timestamp, sender, body
            )
    ...
    async def get(
        ...
        async with self.dbh.pool.acquire() as conn:
            data = await conn.fetch(self._q_get_messages, organization_id, recipient, offset)
